...
ask-session is pre-1.0 (`0.y.z`). Until 1.0.0:

- **PATCH** (`0.1.0` → `0.1.1`) — bug fixes and other
  backwards-compatible corrections. No new features, no public API changes.
- **MINOR** (`0.1.0` → `0.2.0`) — new features **and**
...
Patch numbers increase by exactly one per release. From `0.1.0` the next
patch is `0.1.1` — never `0.1.4`, never a skip, never a jump. Patch
...
Every `ask-*` gem — ask-session included — and `yamine` is released through
...
gemchain guard ask-session                   # blast radius: who depends on us
gemchain update ask-session 0.1.1 --dry-run
gemchain update ask-session 0.1.1 --test-only
gemchain update ask-session 0.1.1
```
...
2. **Version** — bump `lib/ask/session/version.rb` by exactly one step
   (gemchain does this).
...
5. **Build** — `gem build ask-session.gemspec` succeeds and the packaged
...
- `lib/ask/session/version.rb` at `HEAD`
- the `vX.Y.Z` git tag
...
- Fix a bug with no API change → `0.1.1` (patch).
- Add a new session feature → `0.2.0` (minor).
- Remove or rename a public method → `0.2.0` (minor — breaking is allowed
  pre-1.0, but only ever in a minor).
- From `0.1.0`, **never** jump to `0.1.4` for a fix, and **never** release
  `0.3.0` for a one-line bugfix.
- Two fixes in a row → `0.1.1`, then `0.1.2` — one step each time.
- Dependency example: releasing `ask-core` runs
  `gemchain update ask-core <ver>` from the workspace root; the cascade
  rewrites this gem's gemspec constraint, re-runs this suite, bumps
  ask-session by one patch, commits, publishes, tags, and pushes — no
